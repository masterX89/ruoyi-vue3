...
function getGridItemChildren(children) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (child.type === vue.Fragment) {
      if (child.patchFlag & vueUtils.PatchFlags.KEYED_FRAGMENT)
        keyedFragmentCount++;
      ret = ret.concat(getGridItemChildren(child.children));
    } else if (vueUtils.isNamedComponent(child, "GridItem")) {
      ret.push(child);
    }
  }
  if (keyedFragmentCount > 1) {
    for (let i = 0; i < ret.length; i++) {
      ret[i].patchFlag = vueUtils.PatchFlags.BAIL;
    }
  }
  return ret;
}
...
exports.getGridItemChildren = getGridItemChildren;
